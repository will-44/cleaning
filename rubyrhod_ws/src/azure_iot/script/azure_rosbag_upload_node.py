...
def azure_rosbag_upload_node():
    ...
    # initializing node
    rospy.init_node('azure_rosbag_upload_node', anonymous=False)
...
def upload_closed_rosbag_to_cloud(event=None):
    ...
    os.chdir(_bag_log_folder)
    
    files = glob.glob("*[0-9].bag")
    for file in files:
    #for file in glob.glob("*[0-9].bag"):
        subprocess.run("rosbag compress {filename}".format(filename=_bag_log_folder + file), shell=True)
        if store_blob(_bag_log_folder + file) :
            os.remove(_bag_log_folder + file)

            index=file.find(".bag")
            backup_file=file[:index]+".orig"+file[index:]

            os.remove(_bag_log_folder + backup_file)
